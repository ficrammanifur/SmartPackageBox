...
MQTT_PORT = 1883
MQTT_PUB_TOPIC = "package/command"  # Kirim cmd ke ESP32
...
model = genai.GenerativeModel(MODEL_NAME, system_instruction="""[Prompt kurir pintar seperti yang kamu kasih - dialog natural, nama whitelist, sleep/wake 'hallo', output JSON dengan 'cmd': 'set_status'/'open_box'/'sleep', 'tts_text', 'file': 'response.wav' ]""")
...
recognizer.energy_threshold = 300  # Adjust for mic

# ================= ROUTES =================
@app.route('/audio_stream', methods=['POST'])
def audio_stream():
    try:
        raw_pcm = request.get_data()  # Raw bytes from ESP
        if not raw_pcm:
            return jsonify({"error": "No audio"}), 400

        # Convert raw PCM to WAV (16kHz 16bit mono)
        audio = AudioSegment(
            raw_pcm,
            frame_rate=16000,
        ...
        with sr.AudioFile(wav_io) as source:
            ...
            text = recognizer.recognize_google(audio_data, language='id-ID')  # Indonesia
        print(f"👤 STT: {text}")
        ...
        # TTS
        tts = gTTS(tts_text, lang='id', slow=False)
        tts_io = BytesIO()
        ...
        response_path = os.path.join("static", filename)  # Serve from /static
...
# MQTT on_message for boot/status
def on_mqtt_message(client, userdata, msg):
    topic = msg.topic
    payload = msg.payload.decode().strip()
    if topic == MQTT_STATUS_TOPIC and payload == "boot_ready":
        print("🔊 Boot TTS PC: Perangkat siap")  # Optional PC TTS
        # os.system('espeak "Perangkat siap"')  # Kalau mau
